...
#     delay: int = 0
# ) -> List[TextContent]:
#     """Click at specific pixel coordinates (for use with Moondream)"""
#     try:
#         current_page = await ensure_browser()

#         # Click at the specified coordinates
#         await current_page.mouse.click(x, y, button=button, delay=delay)
...
# @mcp.tool()
# async def smart_click_with_moondream(
#     target_object: str,
#     fallback_selector: str = None,
#     take_screenshot_first: bool = True
# ) -> List[TextContent]:
#     """Smart click using Moondream to find elements, with fallback to selector"""
#     try:
#         current_page = await ensure_browser()

#         # Take screenshot if requested
#         if take_screenshot_first:
#             screenshot_bytes = await current_page.screenshot(type='png')
#             screenshot_b64 = base64.b64encode(screenshot_bytes).decode('utf-8')

#             # Save screenshot for Moondream
#             import tempfile
#             import os
#             temp_path = os.path.join(tempfile.gettempdir(), "moondream_screenshot.png")
#             with open(temp_path, 'wb') as f:
#                 f.write(screenshot_bytes)

#         # Try to use Moondream to find the element
#         try:
#             moondream_result = await test_moondream_pointing(
#                 image_path=temp_path if take_screenshot_first else "latest_screenshot.png",
#                 target_object=target_object
#             )

#             if (moondream_result.get("success") and
#                 moondream_result.get("coordinates") and
#                 len(moondream_result["coordinates"]) > 0):

#                 # Use Moondream coordinates
#                 coord = moondream_result["coordinates"][0]
#                 x = coord["pixel"]["x"]
#                 y = coord["pixel"]["y"]
...
#                     'target_object': target_object,
#                     'coordinates': {'x': x, 'y': y},
#                     'moondream_confidence': coord.get('confidence', 'unknown'),
#                     'message': f'Successfully clicked on "{target_object}" using Moondream at ({x}, {y})'
#                 }
#                 return [TextContent(type="text", text=json.dumps(result, indent=2))]

#         except Exception as moondream_error:
#             logger.warning(f"Moondream failed: {moondream_error}")

#         # Fall back to selector-based clicking
#         if fallback_selector:
#             element = current_page.locator(fallback_selector)
#             await element.wait_for(state='visible', timeout=5000)
#             await element.click()
...
#                 'message': f'Moondream failed, clicked using selector: {fallback_selector}'
#             }
#             return [TextContent(type="text", text=json.dumps(result, indent=2))]
...
#         result = {
#             'success': False,
#             'error': f'Could not find "{target_object}" with Moondream and no fallback selector provided'
#         }
#         return [TextContent(type="text", text=json.dumps(result, indent=2))]
...
#         error_result = {
#             'success': False,
#             'error': f'Smart click failed: {str(e)}'
#         }
#         return [TextContent(type="text", text=json.dumps(error_result, indent=2))]
...
def extract_coordinates_from_text(text: str) -> Dict[str, int]:
    """Extract coordinates from Moondream's text response."""
    patterns = [
        r'\((\d+),\s*(\d+)\)',           # (x, y)
        r'\[(\d+),\s*(\d+)\]',           # [x, y]
        r'x[:\s=]+(\d+).*?y[:\s=]+(\d+)', # x: 100, y: 200
        r'(\d+)[,\s]+(\d+)',             # 100, 200
        r'coordinates?[:\s]+(\d+)[,\s]+(\d+)', # coordinates: 100, 200
    ]
...
async def analyze_image_for_element(
    ...
    """
    Analyze image using local Moondream Station to find element coordinates.

    Args:
        image_base64: Base64 encoded image data
        element_name: Name/description of the element to find
        moondream_url: URL of your local Moondream Station API

    Returns:
        List[TextContent]: Analysis results with coordinates if found
    """
    try:
        prompt = f"Find the {element_name} in this image and provide its exact pixel coordinates in the format (x, y)."
        ...
        if response.status_code == 200:
            ...
            moondream_response = response_data.get('response', '') or response_data.get('text', '') or response_data.get('answer', '')
            logging.info(moondream_response)
            coordinates = extract_coordinates_from_text(moondream_response)
            ...
            if coordinates:
                result = {
                    ...
                    'confidence': 0.9,
                    'message': f'Found "{element_name}" at coordinates ({coordinates["x"]}, {coordinates["y"]})',
                    'raw_response': moondream_response
                }
            else:
                result = {
                    'success': False,
                    ...
                    'coordinates': None,
                    'confidence': 0.0,
                    'message': f'Could not extract coordinates for "{element_name}"',
                    'raw_response': moondream_response
                }
        else:
            result = {
                'success': False,
                'error': f'Moondream Station returned status {response.status_code}: {response.text}',
                'element_name': element_name
            }
        ...
        error_result = {
            'success': False,
            'error': f'Could not connect to Moondream Station at {moondream_url}',
            'element_name': element_name
        }
        return [TextContent(type="text", text=json.dumps(error_result, indent=2))]
        ...
        error_result = {
            'success': False,
            'error': 'Moondream Station request timed out',
            'element_name': element_name
        }
        return [TextContent(type="text", text=json.dumps(error_result, indent=2))]
